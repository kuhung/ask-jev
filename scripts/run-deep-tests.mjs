...
async function testAll() {
  const cases = [
    // 1. Yes/No 消费类
    {
      ...
      expected: '返回 Yes 或 No 概率，verdict 非空',
    },
    {
      category: 'Yes/No 消费决策',
      name: '健身房年卡',
    ...
    // 2. Yes/No 社交与行为类 (文案语境检验)
    {
      ...
      expected: '文案应符合社交场景，不应机械出现“坚决别买”或“把钱留在兜里”',
    },
    {
      category: 'Yes/No 社交/行为语义适配',
      name: '情感决策是否复合',
      payload: { question: '要不要和前任复合？', mode: 'yes_no' },
      expected: '文案应符合情感生活语境，避免“买完就贬值”等错位字样',
    },
    {
      category: 'Yes/No 社交/行为语义适配',
      name: '前同事婚礼随礼',
    ...
    },
    {
      ...
      expected: '无“还是/或者”分隔时，应有友好提示或合理提取，而非机械返回“选项A”',
    },
    {
      category: 'Choice 选项解析',
      name: '斜杠多选一',
  ...
  console.log(`执行 ${cases.length} 组深度用例测试...\n`);
  ...
  for (const c of cases) {
    ...
    try {
      ...
      console.log(`[${status}] (${latency}ms) ${c.category} - ${c.name}`);
    } catch (err) {
      const latency = Math.round(performance.now() - start);
      detailedResults.push({
        category: c.category,
      ...
      console.log(`[ERROR] (${latency}ms) ${c.category} - ${c.name}: ${err.message}`);
    }
  }
...
testAll().then((res) => {
  import('fs').then((fs) => {
    fs.writeFileSync('scripts/test-output.json', JSON.stringify(res, null, 2), 'utf-8');
    console.log('\n测试数据已写入 scripts/test-output.json');
  });
});
